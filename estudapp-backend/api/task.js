...
module.exports = app => {
	...
	const save = (req, res) => {
		if(!req.body.description.trim() || !req.body.subject.trim() || !req.body.priority.trim()) {
			return res.status(400).send('Insira dados nos campos obrigatórios')
		}
	...
	const update = (req, res) => {
		app.db('tasks')
			...
			}, ['id', 'subject', 'priority', 'description', 'estimateAt', 'doneAt'])
			.then(_ => res.status(204).send())
			.catch(err => res.status(400).json(err))
	}
